  ...
  async fetch(request, env, ctx) {
    ...
    if (url.searchParams.has("sub")) {
      try {
        ...
        // 稳妥解码：将 Hex 字符串还原为标准 UTF-8 字符串
        let decodedText = "";
        for (let i = 0; i < hexData.length; i += 2) {
          decodedText += String.fromCharCode(parseInt(hexData.substr(i, 2), 16));
        }
        const config = JSON.parse(decodeURIComponent(escape(decodedText)));
        ...
        // 解析 Query 参数并锁定 host/sni
        const searchParams = new URLSearchParams(queryStr);
        if (!searchParams.has("host")) searchParams.set("host", originalHost);
        if (!searchParams.has("sni")) searchParams.set("sni", originalHost);

        // 循环优选 IP 列表生成新节点
        let resultNodes = [];
        ipList.forEach((ipObj, index) => {
          const targetIP = ipObj.ip;
          const targetPort = ipObj.port || originalPort;
        ...
        // 返回 Base64 编码的通用订阅文本流
        return new Response(btoa(unescape(encodeURIComponent(finalPayload))), {
          headers: { 
...
<div class="container">
    ...
    <div id="resultZone" class="result-zone">
        ...
        <div class="hint" style="color: green; font-weight: bold;">请在代理软件中直接添加上方链接即可。</div>
    </div>
</div>
...
<script>
    ...
    function parseIPs(text) {
        ...
        lines.forEach(line => {
            ...
            } else if (!item.includes(']') && item.includes(':') && (item.match(/:/g) || []).length === 1) {
    ...
    function generateSubscription() {
        try {
            ...
            const strData = unescape(encodeURIComponent(JSON.stringify(configData)));
            let hexData = "";
            for (let i = 0; i < strData.length; i++) {
                hexData += strData.charCodeAt(i).toString(16).padStart(2, '0');
            }

            const finalSubUrl = \`\${workerOrigin}/?sub=1&data=\${hexData}\`;

            document.getElementById('resultUrl').innerText = finalSubUrl;
            document.getElementById('resultZone').style.display = 'block';
        } catch (e) {
            alert('前端逻辑发生未知错误: ' + e.message);
        }
    }
