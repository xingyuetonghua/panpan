...
// 功能：粘贴一个原始 VLESS+WS+TLS+CF 节点链接，再批量粘贴优选 IP，一键生成新订阅。
// 安全边界：本 Worker 只返回一个网页，所有转换都在你的浏览器本地完成。
// 它不做反代、不做 WebSocket 中转、不做 Tunnel、不转发任何代理流量。
...
<body>
  <main class="wrap">
    <section class="card">
      ...
      <p>粘贴一个你已经能用的 VLESS+WS+TLS+Cloudflare 节点链接，再批量粘贴优选 IP，即可生成一组新节点订阅。本页面只做文本转换，不转发任何代理流量。</p>
    </section>
    ...
    <section class="card">
      ...
      <div class="hint">要求：最好粘贴一个已经能正常连接的 VLESS + WS + TLS + CF 橙云节点链接。</div>
    </section>
    ...
    <section class="card">
      ...
      <div class="hint">支持每行一个 IP，也支持带空格、逗号的文本，会自动提取 IPv4 / IPv6 / 域名。</div>
    </section>
    ...
    <section class="card">
      ...
      <div class="btns">
        <button onclick="generate()">一键生成订阅</button>
        <button class="secondary" onclick="copyOutput()">复制结果</button>
        <button class="light" onclick="downloadOutput()">下载 sub.txt</button>
    ...
    <section class="card">
      ...
      <div class="hint">把这个结果复制到客户端，或下载为 txt 后放到你自己的订阅地址里。</div>
    </section>
  </main>
  ...
  <script>
    function parseVless(link) {
      ...
      const mainAndQuery = hashIndex >= 0 ? withoutScheme.slice(0, hashIndex) : withoutScheme;
      const qIndex = mainAndQuery.indexOf('?');
      const userHost = qIndex >= 0 ? mainAndQuery.slice(0, qIndex) : mainAndQuery;
      const query = qIndex >= 0 ? mainAndQuery.slice(qIndex + 1) : '';

      const atIndex = userHost.indexOf('@');
      if (atIndex < 0) throw new Error('链接缺少 UUID@地址');

      const uuid = userHost.slice(0, atIndex);
      const hostPort = userHost.slice(atIndex + 1);
      const lastColon = hostPort.lastIndexOf(':');
      const port = lastColon >= 0 ? hostPort.slice(lastColon + 1) : '443';

      const params = new URLSearchParams(query);
      return { uuid, port, params };
    }
    ...
    function extractTargets(text) {
      ...
      const cleaned = raw
        .map(x => x.replace(/^https?:\/\//, '').replace(/\/.*$/, '').replace(/^\[/, '').replace(/\]$/, ''))
    ...
    function formatAddressForVless(address) {
      // IPv6 在 VLESS URL 里需要写成 [IPv6]:端口，否则很多客户端会解析失败
